...
struct fieldIdHash
{
  std::size_t operator()(const SQLSMALLINT& key) const {
    if (key < SQL_DESC_COUNT) {
      return key - 1;
    }
    return key - 966;
  }
...
const std::unordered_map < SQLSMALLINT, std::array<SQLSMALLINT, 4>, fieldIdHash> MADB_DESC_FLDID=
{
...
MADB_SetIrdRecord(MADB_Stmt *Stmt, MADB_DescRecord *Record, const MYSQL_FIELD *Field)
{
  ...
  */
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    ...
    Record->Precision= MIN((SQLSMALLINT)Field->length - MADBTEST(Record->Unsigned == SQL_FALSE) - MADBTEST(Record->Scale > 0),
...
/* {{{ MADB_DescCheckFldId */
bool MADB_DeskCheckFldId(MADB_Desc *Desc, SQLSMALLINT FieldIdentifier, SQLSMALLINT mode)
{
  const auto& it= MADB_DESC_FLDID.find(FieldIdentifier);

  /* End of list= invalid FieldIdentifier */
  if (it == MADB_DESC_FLDID.end() ||
      !(it->second[Desc->DescType] & mode)) {
                            ...
                            int isWChar)
{
  ...
  if (MADB_DeskCheckFldId(Desc, FieldIdentifier, MADB_DESC_READ))
...
/* Checks if application may write to the requested field */
bool MADB_ValidateDescFieldAccess(SQLHDESC DescriptorHandle, SQLSMALLINT FieldIdentifier, SQLPOINTER ValuePtr)
{
  MADB_Desc *Desc= static_cast<MADB_Desc*>(DescriptorHandle);
  if (FieldIdentifier == SQL_DESC_UNNAMED && (SQLSMALLINT)(SQLULEN)ValuePtr == SQL_NAMED)
  {
    MADB_SetError(&Desc->Error, MADB_ERR_HY092, nullptr, 0);
    return true;
  }
  return MADB_DeskCheckFldId(Desc, FieldIdentifier, MADB_DESC_WRITE);
}
