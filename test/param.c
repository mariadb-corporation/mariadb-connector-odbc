...
#define PARAMSET_SIZE		10
  ...
  if (!IsMysql && ServerNotOlderThan(Connection, 10, 2, 7))
    // Tecnichally, the feature is present in all supported server versions. 
  {
  ...
  {
    /* With MySQL driver executes queries one by one, thus errors on some paramsets result on
