...
ODBC_TEST(t_bug30626)
{
  ...
  /* Similarly, MariaDB TIME type can have negative time with up to 859 hours and microseconds.
  ...
  CHECK_STMT_RC(Stmt, SQLFetch(Stmt));
  is_num(my_fetch_int(Stmt, 3), 10/*YYYY-MM-DD*/);
  EXPECT_STMT(Stmt, SQLFetch(Stmt), SQL_NO_DATA);
  CHECK_STMT_RC(Stmt, SQLFreeStmt(Stmt, SQL_CLOSE));
  CHECK_STMT_RC(Stmt, SQLGetTypeInfo(Stmt, SQL_VARCHAR));
  while (SQL_SUCCEEDED(SQLFetch(Stmt)))
  {
    my_fetch_str(Stmt, buf, 1);
    if (strncmp(buf, "SET", 3) && strncmp(buf, "ENUM", 4))
    {
      is_num(my_fetch_int(Stmt, 3), 65532/*max length of string representation for these types*/);
    }
    else
    {
      /* Effective max length of string representation for SET and ENUM */
      is_num(my_fetch_int(Stmt, 3), 65216);
    }
  }
  CHECK_STMT_RC(Stmt, SQLFreeStmt(Stmt, SQL_CLOSE));
  CHECK_STMT_RC(Stmt, SQLGetTypeInfo(Stmt, SQL_VARBINARY));
  CHECK_STMT_RC(Stmt, SQLFetch(Stmt));
  is_num(my_fetch_int(Stmt, 3), 65532);
  CHECK_STMT_RC(Stmt, SQLFreeStmt(Stmt, SQL_CLOSE));
  ...
  CHECK_STMT_RC(Stmt1, SQLFetch(Stmt1));
  is_num(my_fetch_int(Stmt1, 3), 10/*YYYY-MM-DD*/);
  EXPECT_STMT(Stmt1, SQLFetch(Stmt1), SQL_NO_DATA);
  CHECK_STMT_RC(Stmt1, SQLFreeStmt(Stmt1, SQL_CLOSE));
  CHECK_STMT_RC(Stmt1, SQLGetTypeInfo(Stmt1, SQL_VARCHAR));
  while (SQL_SUCCEEDED(SQLFetch(Stmt1)))
  {
    my_fetch_str(Stmt1, buf, 1);
    if (strncmp(buf, "SET", 3) && strncmp(buf, "ENUM", 4))
    {
      is_num(my_fetch_int(Stmt1, 3), 65532/*max length of varchar string*/);
    }
    else
    {
      /* Effective max length of string representation for SET and ENUM */
      is_num(my_fetch_int(Stmt1, 3), 65216);
    }
  }
  CHECK_STMT_RC(Stmt1, SQLFreeStmt(Stmt, SQL_CLOSE));
  CHECK_STMT_RC(Stmt1, SQLGetTypeInfo(Stmt1, SQL_VARBINARY));
  CHECK_STMT_RC(Stmt1, SQLFetch(Stmt1));
  is_num(my_fetch_int(Stmt1, 3), 65532);
  CHECK_STMT_RC(Stmt1, SQLFreeStmt(Stmt1, SQL_CLOSE));
